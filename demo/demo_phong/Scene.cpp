...
struct ShadowAnyHit : public IShader
{
    void operator() (IPayload& payload_, const IAttribute& attrib_) const
    {
...
static void LoadCornellBox(Scene* scene, std::vector<Triangle>& triangles, std::vector<Sphere>& spheres)
{
    vec3 vs[8] = {
        {-0.5f, -0.5f,  0.5f},
        { 0.5f, -0.5f,  0.5f},
...
bool LoadScene(Scene* scene)
{
    ...
    std::shared_ptr<ShadowAnyHit> shadowAnyHit = std::make_shared<ShadowAnyHit>();
    ...
    materialProgram->SetAnyHitProgram<ShadowAnyHit>(1, shadowAnyHit); // 1: shadow ray
    ...
    // setup model(s)
    LoadCornellBox(scene, triangles, spheres);
